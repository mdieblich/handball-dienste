...
function findSpiel(string $meisterschaft, int $spielnr, int $mannschaft_id, int $gegner_id, int $isHeimspiel): ?Spiel{
    global $wpdb;
    $table_name = $wpdb->prefix . 'spiel';
    $result = $wpdb->get_row("SELECT * FROM $table_name WHERE meisterschafts_kuerzel=\"$meisterschaft\" AND spielnr=$spielnr AND mannschaft=$mannschaft_id AND gegner=$gegner_id AND heimspiel=$isHeimspiel", ARRAY_A);
    if(empty($result)){
      return null;
    }
...
function insertSpiel(string $meisterschaft, int $spielnr, int $mannschaft_id, int $gegner_id, bool $isHeimspiel, int $halle, ?DateTime $anwurf){
    global $wpdb;
